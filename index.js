...
app.post('/submit-form', async (req, res) => {
  try {
    ...
    // 1. Direct message to Telegram user
    if (data.chat_id) {
      const clientMessage = `
...
Our team will contact you shortly!
      `;
      await bot.sendMessage(data.chat_id, clientMessage, { parse_mode: 'Markdown' }).catch(err => console.error("Client DM Error:", err));
    }

    // 2. Alert to Telegram Group Topic
    if (GROUP_CHAT_ID) {
      const groupMessage = `
...
📝 *Notes:* ${data.notes || 'None'}
      ...
      await bot.sendMessage(GROUP_CHAT_ID, groupMessage, options).catch(err => console.error("Group Alert Error:", err));
    }

    // 3. Log to Google Sheets without throwing blocking errors
    if (SCRIPT_URL) {
      fetch(SCRIPT_URL, {
        method: 'POST',
